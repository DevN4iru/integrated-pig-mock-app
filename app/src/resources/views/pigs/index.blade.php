...
@section('styles')
/* PIGSTEP PIG LIST COLOR RESTORE START */
.pig-section-filters {
...
.pig-section-batch {
    ...
    background: linear-gradient(180deg, #eff6ff 0%, #ffffff 76%) !important;
    box-shadow: 0 0 0 1px rgba(59, 130, 246, 0.12), 0 16px 34px rgba(59, 130, 246, 0.08) !important;
}

.pig-section-active {
    border-color: #86efac !important;
    background: linear-gradient(180deg, #f0fdf4 0%, #ffffff 78%) !important;
    box-shadow: 0 0 0 1px rgba(34, 197, 94, 0.12), 0 18px 38px rgba(34, 197, 94, 0.08) !important;
}

.pig-section-sold {
    border-color: #86efac !important;
    background: linear-gradient(180deg, #f0fdf4 0%, #ffffff 82%) !important;
}

.pig-section-dead {
    border-color: #fca5a5 !important;
    background: linear-gradient(180deg, #fff1f2 0%, #ffffff 82%) !important;
}

.pig-section-archived {
    border-color: #cbd5e1 !important;
    background: linear-gradient(180deg, #f8fafc 0%, #ffffff 82%) !important;
}
...
.pig-status-sold::before { background: #22c55e; }
.pig-status-dead::before { background: #ef4444; }
.pig-status-archived::before { background: #94a3b8; }

.pig-section-batch .batch-toolbar .btn[disabled] {
    opacity: 0.48;
    cursor: not-allowed;
    background: #f8fafc;
    color: #94a3b8;
...
.pig-list-mobile-note {
    display: none;
}

@media (max-width: 900px) {
    .pig-section-batch .batch-toolbar {
        display: grid !important;
        grid-template-columns: 1fr 1fr;
        gap: 8px;
    }

    .pig-section-batch .batch-toolbar .btn {
        width: 100%;
        justify-content: center;
    }

    .pig-section-active table {
        min-width: 860px;
    }

    .pig-section-active .table-wrap {
        overflow-x: auto;
...
/* PIGSTEP PIG LIST COLOR RESTORE END */
    ...
    <div class="panel-card">
        <div class="section-title">
            <div>
                ...
                <p>Live pigs are now grouped by assigned pen for easier housing operations.<span class="pig-list-mobile-note">Tables scroll sideways on small screens.</span></p>
            </div>
        </div>
...
<script id="pig-list-operation-order">
document.addEventListener('DOMContentLoaded', function () {
    ...
    function firstHeadingText(panel) {
        const h = panel.querySelector('h1, h2, h3, .dashboard-section-title');
        return h ? h.textContent.trim() : '';
    }

    function findPanel(label) {
        return panels.find(function (panel) {
            return firstHeadingText(panel).includes(label);
        });
    }

    function tagPanel(label, cls) {
    ...
    tagPanel('Search & Filters', 'pig-section-filters');
    tagPanel('Batch Actions', 'pig-section-batch');
    tagPanel('Active Pigs by Pen', 'pig-section-active');
    tagPanel('Sold Pigs', 'pig-section-sold');
    tagPanel('Dead Pigs', 'pig-section-dead');
    tagPanel('Archived Pigs', 'pig-section-archived');

    Array.from(document.querySelectorAll('h3')).forEach(function (heading) {
        const t = heading.textContent.trim();

        if (t === 'Sold Pigs') {
            heading.classList.add('pig-status-heading', 'pig-status-sold');
        }

        if (t === 'Dead Pigs') {
            heading.classList.add('pig-status-heading', 'pig-status-dead');
        }

        if (t === 'Archived Pigs') {
            heading.classList.add('pig-status-heading', 'pig-status-archived');
        }
    });

    const orderedPanels = panelLabels.map(findPanel).filter(Boolean);
    const commonParent = orderedPanels[0] ? orderedPanels[0].parentElement : null;

    if (commonParent) {
        orderedPanels.forEach(function (panel) {
            if (panel.parentElement === commonParent) {
    ...
    document.addEventListener('change', function (event) {
        if (
            ...
            window.setTimeout(refreshBatchButtonState, 0);
        }
    });
